---
version: alpha
name: 24visa-makassar-airbnb-style
description: A warm, generous consumer marketplace anchored on a clean white canvas and 24visa True Red (#E31B23), the single brand voltage that carries every primary CTA, search-button orb, and rating dot. Type runs Plus Jakarta Sans at modest weights — display sits at 22–28px in weight 500/600 rather than the heavy 700+ that fintech and enterprise systems use; the brand trusts photography and generous whitespace over typographic muscle. Three product entries (Umrah, Haji, Visa) sit in the top nav with hand-illustrated 32-icon glyphs and "NEW" badges, signaling a marketplace expansion rather than a feature dump. Pill-shaped search bars (`{rounded.full}`), softly rounded property cards (`{rounded.lg}` ~14px), and 32px button radii read as friendly and human — there is no hard corner anywhere except the body grid.

colors:
  primary: "#E31B23"
  primary-active: "#B8141A"
  primary-disabled: "#F7C1C3"
  primary-error-text: "#c13515"
  primary-error-text-hover: "#b32505"
  accent-blue: "#0056B3"
  accent-blue-soft: "#E6F0FA"
  ink: "#222222"
  body: "#3f3f3f"
  muted: "#6a6a6a"
  muted-soft: "#929292"
...
24visa Makassar is the canonical example of a generous, photography-led consumer marketplace for travel and visa services. The base canvas is **pure white** (`{colors.canvas}` — #ffffff) with deep near-black ink (`{colors.ink}` — #222222) for headlines and body, and a single voltage of **True Red** (`{colors.primary}` — #E31B23) carrying every primary CTA, the search-button orb, the heart save state, and inline brand links. We also incorporate **Electric Blue** (`{colors.accent-blue}` — #0056B3) for secondary badges and accents.

Type runs **Plus Jakarta Sans** for everything. Display headlines render at 22–28px in weight 500–600. The hero h1 ("Inspiration for future getaways") on the homepage is just 28px / 700, which works because the layout leans on photography (city collage, property cards) for visual weight rather than typographic muscle.

The shape language is **soft** (Airbnb-inspired). Buttons are 8px radius (`{rounded.sm}`), property cards are ~14px (`{rounded.md}`), the search bar is fully pill-shaped (`{rounded.full}`), wishlist hearts and search orbs are circles (`{rounded.full}`), and category strip rounded corners run at 32px (`{rounded.xl}`). There is essentially no hard corner anywhere except the body grid itself — every interactive element is rounded.

**Key Characteristics:**
- Brand colors: `{colors.primary}` (#E31B23 — "True Red") and `{colors.accent-blue}` (#0056B3 — "Electric Blue"). True Red carries every primary CTA and search orb.
- Custom variable type: `Plus Jakarta Sans`. Display weights sit at 500–700, body at 400. Modest weight is intentional — the system trusts photography for visual heft.
- Three-product top nav: Umrah, Haji, Visa — each with a hand-illustrated 32px icon and "NEW" badges (`{component.new-tag}`).
- Pill-shaped global search bar: white surface, fully rounded (`{rounded.full}`), divided by 1px hairlines.
- Property cards are photo-first: aspect-ratio rectangles with `{rounded.md}` corner clipping, swipeable image carousel.
...
- 8px base spacing system, with major sections at `{spacing.section}` (64px).

## Colors

### Brand & Accent
- **True Red** (`{colors.primary}` — #E31B23): The single brand color. Used for primary CTA backgrounds (Reserve, Continue), the search orb, the heart save state on property cards, and inline brand links.
- **True Red Active** (`{colors.primary-active}` — #B8141A): The press / pointer-down variant — slightly more saturated.
- **True Red Disabled** (`{colors.primary-disabled}` — #F7C1C3): A pale tint used on disabled CTAs.
- **Electric Blue** (`{colors.accent-blue}` — #0056B3): Sub-brand accent for 24visa.
- **Electric Blue Soft** (`{colors.accent-blue-soft}` — #E6F0FA): Sub-brand background accent.
